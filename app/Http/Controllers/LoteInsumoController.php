        ...
        $insumos = Insumo::where('estado', 1)
            ->whereHas('lotes', function ($query) {
                $query->where(function ($q) {
                    $q->where(function ($q2) {
                        $q2->where('insumos.unidadDeMedida', 'gramos')->where('stockActual', '<', 500);
                    })->orWhere(function ($q2) {
                        $q2->where('insumos.unidadDeMedida', 'kilos')->where('stockActual', '<', 1);
                    })->orWhere(function ($q2) {
                        $q2->where('insumos.unidadDeMedida', 'unidades')->where('stockActual', '<', 10);
                    })->orWhere(function ($q2) {
                        $q2->where('insumos.unidadDeMedida', 'litros')->where('stockActual', '<', 2);
                    });
                });
            })->paginate(5);
